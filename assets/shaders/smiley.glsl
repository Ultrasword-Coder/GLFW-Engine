...
void main(){
    ...
    vec4 container = texture(utex1, TexCoord);
    gl_FragColor = mix(u_clearColor, container, u_mixPct);
    
    float smile = cos(utime)*.5+.5;
    vec4 smiley = smiley(uv, m, smile);
    gl_FragColor = mix(gl_FragColor, smiley, smiley.a);
}